...
model{
  ...
  for(i in 1:n) {
    ...
    //target += normal_lpdf(y[i] | alpha + X[i,]*beta, sigma);  // this works too
  }
  //y ~ normal(alpha + X*beta, sigma); // vectorized but not usable with bridge sampling

}
generated quantities{
  real mu[n];
  for(i in 1:n){
    mu[i] = alpha + dot_product(beta, X[i,]);
  }
}
