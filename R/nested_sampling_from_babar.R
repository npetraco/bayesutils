...
#
# Modified slightly to print out progress and return a few more things
#
########################################################################
...
nestedSampling <- function
  ...
  # ***********Storage: Hard to pre-allocate since we don't know how many steps we'll be taking.
  # Is there a penalty in C++ to append with STL containers?
  log.widths <- numeric() # Can we just make these large and see if they are getting filled?
  logLstars  <- numeric() # ONCE FILLED then expand them. At end of calc, trim down.
  ...
  while(maxLeft > logTolerance) {
    ...
    logLstars[samplesCounter]  <- llMin     # ***************Added
    log.widths[samplesCounter] <- log.width # ***************Added
    ...
    #-------------------------------------------------------------------
    if(printQ==T){
      print(paste("Iter:", samplesCounter, 
                  "logZ:", round(logZ,3), 
                  "maxLeft:", round(maxLeft,3), 
                  "maxLeft-logTol:", round(maxLeft-logTolerance, 3)
                 ))
    }
    #-------------------------------------------------------------------
  ...
  return(list(logevidence=unname(logZ), 
              posterior=posterior.samples, 
              logZerror = unname(logZerror), 
              parameterMeans = sampleMean, 
              parameterVariances = sampleVariance, 
              entropy = unname(entropy), 
              logLstars=unname(logLstars), 
              log.widths=unname(log.widths),
              log.weight=unname(log.weight)
            ))
